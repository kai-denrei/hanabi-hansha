...
- Touch the underwater **drone icon** for a synchronized show: hovering sphere, flapping bird, rotating cube, double helix, beating heart, Saturn rings, kaleidoscope flower, and UFO landing. The fleet morphs between animated formations with coordinated colors and subtle hovering. Choose Prism/Aurora/Ember colors, three paces, and whether to repeat; the default cycle lasts about two minutes. Japanese text, emojis, and line breaks can be added as formation nine, shown immediately before the sequence continues. **Land drones** ends the show with a descent.
